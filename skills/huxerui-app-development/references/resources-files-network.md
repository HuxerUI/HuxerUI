...
SVG files compile ahead of time to platform-neutral vector resources. The supported static subset includes paths and basic shapes, groups, file-local `defs`/`use`, one-path `clipPath` geometry in `userSpaceOnUse`, solid fill and stroke styles, transforms, root `preserveAspectRatio`, `currentColor`, static visibility, absolute CSS lengths, and complete stroke configuration including dash arrays. Keep references inside the same file and give every referenced element a unique ID. Browser-dependent text, scripts, external styles or references, gradients, masks, filters, embedded images, animation, percentages, font-relative units, fractional group opacity, and clip unions are rejected rather than approximated. Check the active SDK documentation when consuming an older installed SDK because its compiler may support a smaller subset.
