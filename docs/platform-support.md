...
RootHook-installed macOS PlatformView factories create controlled NSViews under the per-surface `PlatformModules` registry. The adapter retains compatible native instances, applies property revisions, clips and positions them in logical coordinates, and alternates transparent HuxerUI slice views with native containers in final `RenderComposition` order. Shared hit testing routes a point either to the frontmost HuxerUI interaction or to the native subtree without placing native views in a global foreground plane. Focus synchronization and accessibility-anchor substitution remain follow-up work.
...
The macOS-only `example_platform_view` target embeds a controlled native text field, routes native edits through a typed HuxerUI event, updates properties from application State, and exercises native unmount and recreation.
