...
The macOS-only `example_platform_module` target registers a nonvisual AppKit timer factory and exposes it as a typed Root Service. It demonstrates asynchronous Start and Stop calls, ordered Tick events, request cancellation, and native disposal without exposing payloads or wire names to application code.
