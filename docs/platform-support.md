...
Nonvisual module results and events enter a thread-safe FIFO and wake the X11 event loop through `eventfd`, so application callbacks run asynchronously on the Runtime thread without producer-thread reentry.
...
See the [SDK, CLI, Platform Shell, and Module Design](design/sdk-cli.md) for the planned distribution and platform-module model.
