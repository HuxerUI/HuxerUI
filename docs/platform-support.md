...
Registered custom URLs and document opens enter through `application:openURLs:`. Input delivered while AppKit finishes launch is normalized before Runtime construction, and later input enters the same Runtime activation queue. File URLs remain security-scoped `FileReference` capabilities, while non-file URLs preserve their opaque UTF-8 value. URL schemes and document types remain application-owned Info.plist metadata; `example_application` declares `huxerui-example` and text documents for exercising the integration.
...
The application-delegate shell defers a URL-launched Runtime until `openURL` supplies complete document options, maps that value before first composition, and forwards later callbacks into the ordered activation queue. Open-in-place document URLs retain their security-scoped capability in `FileReference`; UIKit copy-before-use deliveries become private read-only temporary snapshots whose lifetime follows the reference. Custom URLs remain opaque UTF-8 values. The repository runner declares `huxerui-example` and text documents, while consumer Xcode projects own their corresponding Info.plist metadata. Scene URL contexts, Universal Links, and Associated Domains remain future native-shell work.
