...
Android binary distribution, `package` and `clean` commands, native module metadata projection, production nonvisual modules, PlatformView hosting on Windows, Linux, and Web, ExternalTexture, iOS device distribution, and OHOS remain proposed. The shared `PlatformPayload`, nonvisual `PlatformInstance` protocol, low-level PlatformView leaf, placement command, unified registry and event routes, `RenderComposition` derivation, platform UI-thread dispatch, macOS NSView hosting, iOS UIView hosting, and Android View hosting with slice composition, shared hit testing, focus traversal, IME coordination, and native accessibility attachment are implemented.
