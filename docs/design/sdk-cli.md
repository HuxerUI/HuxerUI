...
Consumers may request only `COMPONENTS shared` or `COMPONENTS static`; requesting the Linux shared component does not resolve static-only dependencies, while the static component resolves SDL3, SDL3_image, and SDL3_ttf through their CMake packages and GIO and libsoup 3 through pkg-config.
