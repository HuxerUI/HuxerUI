...
Android accepts a retained `android.graphics.Bitmap`, synchronously copies current `GL_TEXTURE_2D` content through EGLImage, or owns a SurfaceTexture/OES consumer behind a producer-facing `android.view.Surface`; Apple retains immutable `CVPixelBufferRef` frames or synchronously copies a completed `MTLTexture`; Windows copies borrowed `PixelFrame` rows or a completed D3D11 texture; Linux copies borrowed `PixelFrame` rows or retains an immutable `GdkTexture`; Web clones an open WebCodecs `VideoFrame` on the browser main thread.
...
The Linux `GdkTexture` implementation retains one immutable GDK frame in its latest-wins mailbox, so producers may publish `GdkGLTexture`, `GdkDmabufTexture`, or another GDK texture without HuxerUI downloading pixels.
...
GDK owns concrete GL and DMA-BUF import, synchronization metadata, format negotiation, and final resource callbacks; HuxerUI does not expose file descriptors, GL names, EGL objects, Vulkan objects, or display-server handles in Runtime or PaintCommand.
