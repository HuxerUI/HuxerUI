...
Interactive built-ins derive hover, focus, pressed, disabled, and ripple or state-overlay treatment from the nearest Theme. Pointer and keyboard activation share the same semantic state transitions. `ButtonStyle::indication` can replace the theme-wide indication when a filled container requires a different state-layer color. Component-owned transparent actions, such as Dialog actions and Menu items, likewise use the indication stored in their component style.
