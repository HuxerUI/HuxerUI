...
public:
  [[nodiscard]] std::optional<double> Request(double deadline, double now, bool native_ready) noexcept {
    ...
    if (paint_pending_ || paint_in_progress_ || !native_ready) {
  ...
  [[nodiscard]] std::optional<double> EndPaint(bool native_ready) noexcept {
    paint_in_progress_ = false;
    paint_pending_ = false;
    return TakeDeferred(native_ready);
  }

  [[nodiscard]] std::optional<double> TakeDeferred(bool native_ready) noexcept {
