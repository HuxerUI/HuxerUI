...
ScrollBarStyle ResolveScrollBarStyle(
  ...
  if (const std::any *value = detail::FindThemeStyleValue(
          environment, typeid(ScrollBarStyleKey))) {
    if (const auto *style =
            std::any_cast<ScrollBarStyle>(value)) {
      return *style;
    }
    throw std::logic_error(
        "HuxerUI scroll bar style environment value has an invalid type");
  }
  const ThemeSpec theme = detail::ResolveThemeSpec(environment);
  ScrollBarStyle style = ScrollBarStyleKey::Default();
  style.fade_in_duration = theme.motion.reduced_motion
                               ? 0.0F
                               : static_cast<float>(theme.motion.fast);
  style.fade_out_duration = theme.motion.reduced_motion
                                ? 0.0F
...
std::optional<detail::ScrollBarGeometry>
ResolvePresentedScrollBarGeometry(MountedNode &node) {
  auto geometry = detail::ResolveScrollBarGeometry(
      static_cast<detail::MountedNode &>(node));
  if (!geometry.has_value()) {
    return std::nullopt;
  }
  const Rect layout_frame = node.Frame();
  const Rect presentation_frame = node.PresentationFrame();
  const float offset_x =
      presentation_frame.x - layout_frame.x;
  const float offset_y =
      presentation_frame.y - layout_frame.y;
  geometry->track.x += offset_x;
  geometry->track.y += offset_y;
  geometry->thumb.x += offset_x;
  geometry->thumb.y += offset_y;
  return geometry;
}

class MountedScrollBar final : public MountedModifier {
public:
  MountedScrollBar(MountedNode &node, const ScrollBar &modifier) {
  ...
  void Update(MountedNode &node, const ScrollBar &modifier) {
    ...
    style_ = ResolveScrollBarStyle(
        mounted.environment, modifier.style);
  }
  ...
  ModifierFrameResult OnFrame(
    ...
    if (!node.IsEnabled() ||
        !detail::ResolveScrollBarGeometry(mounted).has_value()) {
      opacity_.Set(0.0F);
      initialized_ = false;
      return {};
    }
    ...
    if (activity_pending_) {
      opacity_.AnimateTo(
          1.0F, frame.timestamp, style_.fade_in_duration);
      hide_deadline_ = frame.timestamp + style_.fade_out_delay;
    ...
    const bool held =
        hovered_ || pointer_dragging_ || scroll_dragging_;
    if (held) {
      opacity_.AnimateTo(
          1.0F, frame.timestamp, style_.fade_in_duration);
    } else if (frame.timestamp >= hide_deadline_) {
      opacity_.AnimateTo(
          0.0F, frame.timestamp, style_.fade_out_duration);
    }

    opacity_.Advance(frame.timestamp);
    ...
    if (!held && opacity_.Value() > 0.0F &&
        frame.timestamp < hide_deadline_) {
      return {
          false,
  ...
  bool HitTest(MountedNode &node, Point position) const override {
    const auto geometry =
        ResolvePresentedScrollBarGeometry(node);
    return node.IsEnabled() &&
           geometry.has_value() && opacity_.Value() > 0.01F &&
  ...
  ModifierPointerResult
  OnPointer(MountedNode &node, const PointerEvent &event) override {
    auto &mounted = static_cast<detail::MountedNode &>(node);
    if (!node.IsEnabled()) {
      pointer_id_.reset();
    ...
    if (event.type == PointerEventType::Down) {
      ...
      pointer_draggable_ =
          ...
          geometry->thumb.Contains(event.position);
      pointer_dragging_ = true;
      activity_pending_ = true;
      return ModifierPointerResult::Capture;
    }
    ...
    if (event.type == PointerEventType::Move &&
        pointer_draggable_ && thumb_travel_ > 0.0F) {
      const float pointer =
          pointer_axis_ == Axis::Vertical
              ? event.position.y
              : event.position.x;
      const float desired = std::clamp(
          offset_origin_ +
              (pointer - pointer_origin_) *
                  maximum_offset_ / thumb_travel_,
          0.0F, maximum_offset_);
      const float current =
          pointer_axis_ == Axis::Vertical
              ? mounted.scroll_offset_y
              : mounted.scroll_offset_x;
      if (detail::ScrollNodeBy(mounted, desired - current) !=
          0.0F) {
        activity_pending_ = true;
      }
  ...
  void Paint(
    ...
    const auto geometry =
        ResolvePresentedScrollBarGeometry(mutable_node);
    ...
    track_color.alpha *=
        opacity_.Value() * node.PresentationOpacity();
    Color thumb_color = geometry->style.thumb_color;
    thumb_color.alpha *=
        opacity_.Value() * node.PresentationOpacity();
    if (track_color.alpha > 0.0F) {
      display_list.DrawRect(
          geometry->track, track_color,
          geometry->style.corner_radius);
    }
    if (thumb_color.alpha > 0.0F) {
      display_list.DrawRect(
          geometry->thumb, thumb_color,
          geometry->style.corner_radius);
    }
  }
...
void ValidateScrollBarStyle(const ScrollBarStyle &style) {
  if (!std::isfinite(style.thickness) || style.thickness <= 0.0F ||
      !std::isfinite(style.minimum_thumb_extent) ||
      style.minimum_thumb_extent <= 0.0F ||
      !std::isfinite(style.margin) || style.margin < 0.0F ||
      !std::isfinite(style.corner_radius) || style.corner_radius < 0.0F ||
      !std::isfinite(style.fade_in_duration) ||
      style.fade_in_duration < 0.0F ||
      !std::isfinite(style.fade_out_delay) ||
      style.fade_out_delay < 0.0F ||
      !std::isfinite(style.fade_out_duration) ||
      style.fade_out_duration < 0.0F) {
    throw std::invalid_argument(
        "HuxerUI scroll bar style values must be finite and valid");
  }
}

void ApplyScrollBar(
    detail::ViewSpec &spec, const ScrollBar &modifier) {
  const ScrollBarStyle style =
      ResolveScrollBarStyle(spec.environment, modifier.style);
  ValidateScrollBarStyle(style);
  spec.layout_values.insert_or_assign(
      typeid(detail::ScrollBarBinding), style);
}
...
const detail::ModifierDescriptor &ScrollBar::Descriptor() {
  static const detail::ModifierDescriptor descriptor{
      ...
      [](detail::ViewSpec &spec, const void *value) {
        ApplyScrollBar(
            spec, *static_cast<const ScrollBar *>(value));
      },
      [](MountedNode &node,
         const void *value) -> std::unique_ptr<MountedModifier> {
        return std::make_unique<MountedScrollBar>(
            node, *static_cast<const ScrollBar *>(value));
      },
      [](MountedModifier &mounted, MountedNode &node,
         const void *value) {
        static_cast<MountedScrollBar &>(mounted).Update(
            node, *static_cast<const ScrollBar *>(value));
      },
  };
  return descriptor;
