...
class MountedToastLifetime final : public MountedModifier {
public:
  MountedToastLifetime(
      MountedNode &node, const ToastLifetime &modifier) {
  ...
  ModifierFrameResult OnFrame(
    ...
    const double remaining =
        duration_ - (frame.timestamp - *started_at_);
    if (remaining > 0.0) {
      return {
          false,
          remaining,
...
const detail::ModifierDescriptor &ToastLifetime::Descriptor() {
  return detail::ModifierDescriptorFor<
      ToastLifetime, MountedToastLifetime>();
}
...
typename Key::Value ResolvePresentationStyle(
  ...
  if (const std::any *value = detail::FindThemeStyleValue(
    ...
    throw std::logic_error(
        "HuxerUI presentation style environment value has an invalid type");
  }
  return fallback;
}

ToastStyle ResolveToastStyle(
    const std::shared_ptr<const detail::EnvironmentFrame> &environment) {
  return ResolvePresentationStyle<ToastStyleKey>(
      environment,
      DefaultToastStyle(detail::ResolveThemeSpec(environment)));
}

DialogStyle ResolveDialogStyle(
    const std::shared_ptr<const detail::EnvironmentFrame> &environment) {
...
std::shared_ptr<DialogService> DialogServiceFor(
  ...
  if (!service || !*service) {
    throw std::logic_error(
        "HuxerUI dialog service environment value is invalid");
  }
  return *service;
}

LayerOptions DialogLayerOptions(
  ...
  return {
      ...
      .dismiss_on_outside_press =
          options.dismiss_on_outside_press,
      .on_dismiss_request =
          std::move(options.on_dismiss_request),
      .modal_scrim =
          ResolveDialogStyle(environment).scrim,
  };
}

class MountedDialog final : public MountedModifier {
public:
  MountedDialog(MountedNode &node, const Dialog &modifier) {
    Update(node, modifier);
  }

  ~MountedDialog() override {
...
struct DialogModifierAccess {
  static LayerId Show(
      DialogService &service, ViewFactory content,
      DialogOptions options,
      std::shared_ptr<const EnvironmentFrame> environment) {
    return service.Show(
        std::move(content), options, std::move(environment));
  }

  static bool Update(
      DialogService &service, LayerId id,
      ViewFactory content, DialogOptions options,
      std::shared_ptr<const EnvironmentFrame> environment) {
    return service.Update(
        id, std::move(content), std::move(options),
...
void MountedDialog::Update(
    MountedNode &node, const Dialog &modifier) {
  ...
  if (!modifier.content) {
    throw std::invalid_argument(
        "HuxerUI visible Dialog modifier content must not be empty");
  }
  if (modifier.dismiss_on_outside_press &&
      !modifier.on_dismiss_request) {
    throw std::invalid_argument(
  ...
  DialogOptions options{
      .dismiss_on_outside_press =
          modifier.dismiss_on_outside_press,
      .on_dismiss_request =
          modifier.on_dismiss_request,
  };
  if (layer_.has_value()) {
    detail::DialogModifierAccess::Update(
        *service_, *layer_, modifier.content,
        std::move(options), mounted.environment);
    return;
  }
  layer_ = detail::DialogModifierAccess::Show(
      *service_, modifier.content,
      std::move(options),
      mounted.environment);
}

LayerId ToastHandle::Show(
    std::string message, ToastOptions options) const {
  return service_->Show(
      std::move(message), options, environment_);
}
...
LayerId ToastService::Show(
  ...
  if (!std::isfinite(options.duration) || options.duration < 0.0) {
    throw std::invalid_argument(
        "HuxerUI toast duration must be finite and non-negative");
  }
...
LayerId DialogHandle::Show(
  ...
  return service_->Show(
      std::move(content), options, environment_);
}
...
LayerId DialogService::Show(
  ...
  const LayerOptions layer_options =
      DialogLayerOptions(options, environment);
  return layers_.AttachCaptured(
      layer_options,
      std::move(content), std::move(environment));
}
...
LayerId DialogService::Show(
  ...
  if (!content) {
    throw std::invalid_argument(
        "HuxerUI dialog content factory must not be empty");
  }
  auto id = std::make_shared<LayerId>(0);
  const LayerOptions layer_options =
      DialogLayerOptions(options, environment);
...
bool DialogService::Update(
    LayerId id, ViewFactory content) {
...
bool DialogService::Update(
    ...
    DialogOptions options,
    std::shared_ptr<const detail::EnvironmentFrame> environment) {
  return layers_.Update(
      id, DialogLayerOptions(
              std::move(options), environment),
      std::move(content));
}

bool DialogService::Update(
    LayerId id, DialogFactory content) {
  if (!content) {
    throw std::invalid_argument(
        "HuxerUI dialog content factory must not be empty");
  }
...
const detail::ModifierDescriptor &Dialog::Descriptor() {
  return detail::ModifierDescriptorFor<Dialog, MountedDialog>();
}
