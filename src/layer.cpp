...
LayerController::LayerController(Runtime& runtime)
    : state_(std::make_shared<detail::LayerControllerState>(detail::LayerControllerState{&runtime})) {}
...
LayerId LayerController::AttachCaptured(
  ...
  return state_->runtime->AttachLayer(options, std::move(content), std::move(environment));
}

bool LayerController::Update(LayerId id, ViewFactory content) const {
  return state_->runtime != nullptr && state_->runtime->UpdateLayer(id, std::move(content));
}

bool LayerController::Update(LayerId id, LayerOptions options, ViewFactory content) const {
  return state_->runtime != nullptr && state_->runtime->UpdateLayer(id, std::move(options), std::move(content));
}

bool LayerController::Dismiss(LayerId id) const {
  return state_->runtime != nullptr && state_->runtime->DismissLayer(id);
}
