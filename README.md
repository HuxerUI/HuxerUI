...
payloads provisioned rather than assumed, and `mcpp pack --format
msi|appimage|app|apk|web` produces what a user installs.
...
See [Six Platforms with mcpp](docs/guide/mcpp.md) for the path from environment
to distribution, [C++20/23 modules and mcpp](skills/huxerui-app-development/references/cpp-modules-and-mcpp.md)
for what the code looks like, and the three worked applications in [`mcpp/examples`](mcpp/examples/).
