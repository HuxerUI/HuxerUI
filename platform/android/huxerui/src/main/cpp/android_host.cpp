...
jint PackColor(Color color) {
  ...
  return static_cast<jint>(channel(color.alpha) << 24U | channel(color.red) << 16U | channel(color.green) << 8U |
                           channel(color.blue));
}
...
jbyteArray ToByteArray(JNIEnv *environment, std::string_view text) {
  ...
  environment->SetByteArrayRegion(bytes, 0, static_cast<jsize>(text.size()),
...
public:
  ...
  void Render(JNIEnv *environment, jobject canvas, const DisplayList &display_list) {
    for (const DrawCommand &command : display_list.Commands()) {
      std::visit([this, environment, canvas](const auto &value) { RenderCommand(environment, canvas, value); },
                 command);
      if (environment->ExceptionCheck()) {
        return;
      }
...
private:
  ...
  void RenderCommand(JNIEnv *environment, jobject canvas, const DrawRectCommand &command) {
    environment->CallVoidMethod(view_, draw_rect_, canvas, command.rect.x, command.rect.y, command.rect.width,
                                command.rect.height, PackColor(command.color), command.corner_radius);
  }
  ...
  void RenderCommand(JNIEnv *environment, jobject canvas, const DrawTextCommand &command) {
    ...
    environment->CallVoidMethod(view_, draw_text_, canvas, bytes, command.rect.x, command.rect.y, command.rect.width,
                                command.rect.height, PackColor(command.color), command.font_size,
                                static_cast<jint>(command.align));
    environment->DeleteLocalRef(bytes);
  }

  void RenderCommand(JNIEnv *environment, jobject canvas, const DrawCircleCommand &command) {
    environment->CallVoidMethod(view_, draw_circle_, canvas, command.center.x, command.center.y, command.radius,
                                PackColor(command.color));
  }

  void RenderCommand(JNIEnv *environment, jobject canvas, const DrawArcCommand &command) {
    environment->CallVoidMethod(view_, draw_arc_, canvas, command.center.x, command.center.y, command.radius,
                                command.start_angle * kRadiansToDegrees, command.sweep_angle * kRadiansToDegrees,
                                PackColor(command.color), command.width, static_cast<jint>(command.cap));
  }

  void RenderCommand(JNIEnv *environment, jobject canvas, const DrawBorderCommand &command) {
    environment->CallVoidMethod(view_, draw_border_, canvas, command.rect.x, command.rect.y, command.rect.width,
                                command.rect.height, PackColor(command.color), command.width, command.corner_radius);
  }

  void RenderCommand(JNIEnv *environment, jobject canvas, const PushClipCommand &command) {
    environment->CallVoidMethod(view_, push_clip_, canvas, command.rect.x, command.rect.y, command.rect.width,
                                command.rect.height, command.corner_radius);
  }
...
extern "C" JNIEXPORT jlong JNICALL Java_org_huxerui_HuxerUIView_nativeCreate(JNIEnv *environment, jclass,
                                                                             jobject view) {
  try {
    auto session = std::make_unique<huxerui::detail::AndroidSession>(environment, view,
...
extern "C" JNIEXPORT void JNICALL Java_org_huxerui_HuxerUIView_nativeResize(JNIEnv *environment, jclass, jlong handle,
...
extern "C" JNIEXPORT void JNICALL Java_org_huxerui_HuxerUIView_nativeDraw(JNIEnv *environment, jclass, jlong handle,
                                                                          jobject canvas) {
  try {
    if (auto *session = huxerui::detail::Session(handle)) {
...
extern "C" JNIEXPORT void JNICALL Java_org_huxerui_HuxerUIView_nativeScroll(JNIEnv *environment, jclass, jlong handle,
...
extern "C" JNIEXPORT void JNICALL Java_org_huxerui_HuxerUIView_nativeKey(JNIEnv *environment, jclass, jlong handle,
                                                                         jboolean down, jint key_code, jbyteArray text,
                                                                         jboolean shift, jboolean control, jboolean alt,
                                                                         jboolean meta, jboolean repeat) {
  try {
    if (auto *session = huxerui::detail::Session(handle)) {
      session->KeyEvent(down ? huxerui::KeyEventType::Down : huxerui::KeyEventType::Up, key_code,
