...
std::string Utf8(NSString* value, const char* description) {
  if (value == nil) {
    throw std::runtime_error(std::string("HuxerUI iOS file system could not resolve ") + description);
  }
  const char* utf8 = value.UTF8String;
  if (utf8 == nullptr) {
    throw std::runtime_error(std::string("HuxerUI iOS file system returned an invalid ") + description);
  }
  return utf8;
}
...
NSURL* DirectoryURL(NSFileManager* manager, NSSearchPathDirectory directory, const char* description) {
  ...
  if (url == nil) {
    ...
    throw std::runtime_error(
        std::string("HuxerUI iOS file system could not resolve ") + description +
...
std::shared_ptr<FileSystem> CreateIosFileSystem() {
  @autoreleasepool {
    ...
    NSURL* data = DirectoryURL(manager, NSApplicationSupportDirectory, "the Application Support directory");
    NSURL* cache = DirectoryURL(manager, NSCachesDirectory, "the Caches directory");
