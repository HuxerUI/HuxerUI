...
void AppKitRenderer::RenderCommand(CGContextRef context, const DrawLinearGradientCommand& command) {
...
void AppKitRenderer::RenderCommand(CGContextRef context, const DrawRadialGradientCommand& command) {
  const CFRef<CGGradientRef> gradient = CreateGradient(command.gradient.stops);
  if (gradient.Get() == nullptr || command.rect.IsEmpty()) {
    return;
  }
