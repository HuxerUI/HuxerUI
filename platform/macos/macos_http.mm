...
class MacHttpRequest final : public std::enable_shared_from_this<MacHttpRequest> {
public:
  MacHttpRequest(NSURLSession* session, HttpRequest request, HttpTransportCompletion completion)
...
public:
  ...
  std::function<void()> Start(HttpRequest request, HttpTransportCompletion completion) override {
    auto operation = std::make_shared<MacHttpRequest>(session_, std::move(request), std::move(completion));
    operation->Start();
