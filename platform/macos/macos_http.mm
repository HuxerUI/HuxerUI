...
@interface HuxerUIMacHttpDelegate : NSObject <NSURLSessionDataDelegate> {
  std::weak_ptr<huxerui::detail::MacHttpRequest> request_;
}

- (instancetype)initWithRequest:(std::weak_ptr<huxerui::detail::MacHttpRequest>)request;
...
class MacHttpRequest final : public HttpTransportOperation, public std::enable_shared_from_this<MacHttpRequest> {
public:
  MacHttpRequest(HttpRequest request, HttpTransportCallbacks callbacks)
      : request_(std::move(request)), callbacks_(std::move(callbacks)) {}

  ~MacHttpRequest() override {
    Cancel();
  }
  ...
  void Start() noexcept {
    @autoreleasepool {
      try {
        ...
        std::weak_ptr<MacHttpRequest> weak = shared_from_this();
        HuxerUIMacHttpDelegate* delegate = [[HuxerUIMacHttpDelegate alloc] initWithRequest:weak];
        ...
        FinishError(HttpError{
            ...
            "HuxerUI macOS HTTP request could not be started: " + std::string(exception.what()),
        }, true);
      } catch (...) {
        FinishError(HttpError{HttpErrorCode::Transport, "HuxerUI macOS HTTP request could not be started"}, true);
      }
    }
  }
...
private:
  ...
  __strong HuxerUIMacHttpDelegate* delegate_ = nil;
...
public:
  ...
  Start(HttpRequest request, bool, HttpTransportCallbacks callbacks) override {
    auto operation = std::make_shared<MacHttpRequest>(std::move(request), std::move(callbacks));
    operation->Start();
    return operation;
  }
};
...
std::shared_ptr<HttpTransport> CreateMacHttpTransport() {
  return std::make_shared<MacHttpTransport>();
}

} // namespace huxerui::detail

@implementation HuxerUIMacHttpDelegate
