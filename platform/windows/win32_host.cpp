...
std::string TranslateKeyText(WPARAM virtual_key, LPARAM key_data) {
  ...
  const UINT scan_code =
      static_cast<UINT>((static_cast<std::uintptr_t>(key_data) >> 16U) & 0xFFU);
  const int length = ToUnicodeEx(
      static_cast<UINT>(virtual_key), scan_code, keyboard_state,
      characters, static_cast<int>(std::size(characters)), 0,
      GetKeyboardLayout(0));
  if (length <= 0) {
    return {};
  }
  return WideToUtf8(
      std::wstring_view(characters, static_cast<std::size_t>(length)));
}
...
public:
    ...
    try {
      ...
      if (message_result < 0 && !failure_) {
        failure_ = std::make_exception_ptr(
            std::runtime_error("HuxerUI Windows message loop failed"));
      }
  ...
  void RequestFrame(double delay_seconds) override {
    ...
    if (!std::isfinite(delay_seconds) || delay_seconds <= 0.0) {
      ...
      if (!render_message_posted_) {
        render_message_posted_ =
            PostMessageW(window_, kRenderMessage, 0, 0) != FALSE;
      }
      return;
    }
    ...
    timer_armed_ =
        SetTimer(
            window_, kFrameTimer, static_cast<UINT>(bounded), nullptr) != 0;
  }
  ...
  Size MeasureText(
      std::string_view text, float font_size, float max_width) override {
    ...
    format->SetWordWrapping(
        constrained ? DWRITE_WORD_WRAPPING_WRAP
                    : DWRITE_WORD_WRAPPING_NO_WRAP);

    const float layout_width =
        constrained ? max_width : std::numeric_limits<float>::max();
    ComPtr<IDWriteTextLayout> layout;
    ThrowIfFailed(
        write_factory_->CreateTextLayout(
            wide.data(), static_cast<UINT32>(wide.size()), format.Get(),
            layout_width, std::numeric_limits<float>::max(),
            layout.GetAddressOf()),
    ...
    ThrowIfFailed(
        ...
        "HuxerUI could not measure a DirectWrite text layout");
    const float width = constrained
                            ? std::min(metrics.widthIncludingTrailingWhitespace,
                                       max_width)
...
private:
  ...
  void RegisterWindowClass() {
    ...
    if (class_atom_ == 0) {
      throw std::runtime_error(
          "HuxerUI could not register its Windows window class");
    }
  }
  ...
  void CreateApplicationWindow(const AppOptions &options) {
    ...
    if (!AdjustWindowRectExForDpi(
            &frame, style, FALSE, 0, static_cast<UINT>(dpi_))) {
      throw std::runtime_error(
          "HuxerUI could not calculate the Windows window size");
    }
    ...
    if (window_ == nullptr) {
      throw std::runtime_error(
          "HuxerUI could not create its Windows application window");
    }
  ...
  ComPtr<IDWriteTextFormat> CreateTextFormat(float font_size) const {
    ...
    if (GetUserDefaultLocaleName(
            locale_name, static_cast<int>(std::size(locale_name))) == 0) {
    ...
    ThrowIfFailed(
        write_factory_->CreateTextFormat(
            ...
            std::max(font_size, 0.1F), locale_name, format.GetAddressOf()),
  ...
  bool EnsureRenderTarget() {
    ...
    const HRESULT target_result = d2d_factory_->CreateHwndRenderTarget(
        ...
        D2D1::HwndRenderTargetProperties(
            window_, D2D1::SizeU(width, height)),
  ...
  void Render(const DisplayList &display_list) {
    ...
    render_target_->Clear(
        D2D1::ColorF(247.0F / 255.0F, 248.0F / 255.0F,
                     250.0F / 255.0F, 1.0F));
  ...
  void RenderCommand(const DrawRectCommand &command) {
    ...
    if (command.corner_radius > 0.0F) {
      render_target_->FillRoundedRectangle(
          D2D1::RoundedRect(
              ToD2DRect(command.rect),
              command.corner_radius, command.corner_radius),
          brush_.Get());
    } else {
      render_target_->FillRectangle(
          ToD2DRect(command.rect), brush_.Get());
    }
  }
  ...
  void RenderCommand(const DrawTextCommand &command) {
    ...
    format->SetWordWrapping(
        command.align == TextAlign::Leading
    ...
    render_target_->DrawTextLayout(
        ...
        layout.Get(), brush_.Get(), D2D1_DRAW_TEXT_OPTIONS_CLIP);
  }
  ...
  void RenderCommand(const DrawCircleCommand &command) {
    ...
    render_target_->FillEllipse(
        D2D1::Ellipse(
            D2D1::Point2F(command.center.x, command.center.y),
  ...
  void RenderCommand(const DrawArcCommand &command) {
    ...
    ComPtr<ID2D1StrokeStyle> stroke_style =
        CreateStrokeStyle(command.cap);
    if (std::abs(command.sweep_angle) >= kFullCircle - 0.0001F) {
      render_target_->DrawEllipse(
          D2D1::Ellipse(
              D2D1::Point2F(command.center.x, command.center.y),
              command.radius, command.radius),
          brush_.Get(), command.width, stroke_style.Get());
      return;
    }

    const D2D1_POINT_2F start{
        command.center.x +
            std::cos(command.start_angle) * command.radius,
        command.center.y +
            std::sin(command.start_angle) * command.radius,
    };
    ...
    if (FAILED(d2d_factory_->CreatePathGeometry(
    ...
    sink->AddArc(D2D1::ArcSegment(
        ...
        std::abs(command.sweep_angle) > 3.14159265358979323846F
            ? D2D1_ARC_SIZE_LARGE
            : D2D1_ARC_SIZE_SMALL));
    ...
    render_target_->DrawGeometry(
        geometry.Get(), brush_.Get(), command.width, stroke_style.Get());
  }
  ...
  void RenderCommand(const DrawBorderCommand &command) {
    ...
    render_target_->DrawRoundedRectangle(
        ...
        brush_.Get(), command.width);
  }

  void RenderCommand(const PushClipCommand &command) {
    if (command.corner_radius <= 0.0F) {
      render_target_->PushAxisAlignedClip(
          ToD2DRect(command.rect), D2D1_ANTIALIAS_MODE_PER_PRIMITIVE);
    ...
    ComPtr<ID2D1Layer> layer;
    if (FAILED(render_target_->CreateLayer(
            nullptr, layer.GetAddressOf()))) {
      render_target_->PushAxisAlignedClip(
          ToD2DRect(command.rect), D2D1_ANTIALIAS_MODE_PER_PRIMITIVE);
      clip_stack_.push_back({});
      return;
  ...
  void SendKey(KeyEventType type, WPARAM virtual_key, LPARAM key_data) {
    runtime_->HandleKeyEvent({
        ...
        type == KeyEventType::Down
            ? TranslateKeyText(virtual_key, key_data)
            : std::string{},
        CurrentKeyModifiers(),
        type == KeyEventType::Down &&
            (static_cast<std::uintptr_t>(key_data) & (1ULL << 30U)) != 0,
    });
  }
  ...
  LRESULT HandleMessage(
    ...
    case WM_MOUSEWHEEL: {
      const float delta =
          ...
          static_cast<float>(WHEEL_DELTA) * 120.0F;
  ...
  static LRESULT CALLBACK WindowProcedure(
      HWND window, UINT message, WPARAM w_param, LPARAM l_param) {
    Win32PlatformHost *host = reinterpret_cast<Win32PlatformHost *>(
