...
struct WindowsColorStreamState {
  WindowsColorStreamState() : source({320.0F, 180.0F}) {}
...
huxerui::PlatformModuleFactory WindowsColorStreamFactory() {
  ...
  factory.create = [](const huxerui::PlatformPayload& options, huxerui::PlatformEventSink events) {
    ...
    auto state = std::make_shared<WindowsColorStreamState>();
...
void InstallColorStream(RootContext& root) {
  root.Modules().Register(color_stream::type, WindowsColorStreamFactory());
